...
>
> 1. **Visual recipes first, SQL recipes second, Python recipes ONLY as last resort.** The tier is strict and justified by performance — Python recipes break engine push-down by pulling rows through DSS memory, which is fatal on real workloads. Reach for SQL recipes when window functions, LAG, or complex CTE-based logic beat a visual recipe chain. Reach for Python ONLY when neither visual nor SQL can express the semantics (true PDV state machines, hash-object lookups that can't be reframed as joins, engine without `PERCENTILE_CONT`, etc.). Before writing any Python, ask: "could a Window + Join + Prepare chain do this?"
