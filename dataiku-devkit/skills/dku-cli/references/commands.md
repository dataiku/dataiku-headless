...
- `info --recompute` (alias `--fresh`) recomputes the row count, size, and file count metrics before reading them. **DSS does not auto-recompute metrics on build** — pass `--recompute` (alias `--fresh`) after a recipe run to avoid stale numbers in the cache
