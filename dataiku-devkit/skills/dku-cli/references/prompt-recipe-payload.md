...
> **Workflow**: [create](../SKILL.md#prompt-recipe-programmatic-creation) the recipe shell → `set-settings` with this payload → `dku job run --auto-update-schema`. The `--auto-update-schema` flag is mandatory on first build; `recipe run` alone returns an empty schema.
...
| Key | Required | Value |
|---|---|---|
| `promptMode` | **yes** | `"PROMPT_TEMPLATE_TEXT"` (simple text templates, documented here) or `"PROMPT_TEMPLATE_STRUCTURED"` (chat messages — use `chatMessages` instead). Default to TEXT. |
...
| `chatMessages` | STRUCTURED only | `{}` in TEXT mode. Used by `PROMPT_TEMPLATE_STRUCTURED`. |
| `structuredPromptExamples` | no | `[]` unless using few-shot examples in STRUCTURED mode. |
