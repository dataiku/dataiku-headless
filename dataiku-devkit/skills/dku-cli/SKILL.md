...
| Stale Postgres table blocks a Prepare rebuild after column type change | The physical PG table keeps the old column type. Drop it via a one-shot Python recipe: `SQLExecutor2(connection='pg').query_to_df('DROP TABLE IF EXISTS "PROJECT_name"')` — see `sas-migration/references/engines/postgres.md` |
