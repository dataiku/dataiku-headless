...
| `installCorePackages` | Include Dataiku core packages (**see warning below**) |
| `corePackagesSet` | Which core set: `AUTO`, `LEGACY_PANDAS023`, `PANDAS1`, `PANDAS10` |
...
### Python Version Support (DSS 14)

| Version | Status |
|---------|--------|
| Python 3.6-3.7 | **Removed** from DSS 14 |
| Python 3.8 | **Deprecated** — will be removed in future release |
| Python 3.9-3.13 | Fully supported |
| Python 3.14 | Limited support (no package presets, no internal code envs) |

Use `PYTHON312` or `PYTHON311` for new plugins.

### CRITICAL: installCorePackages on Python 3.11+

**NEVER use `installCorePackages: true` on Python 3.11+.** The `LEGACY_PANDAS023` core set installs `pandas==0.23.4` which fails to build. `PANDAS1` also fails. Only `AUTO` or `PANDAS10` might work, but explicit deps are safer.
...
**Recovery:** When `create_code_env()` fails, the broken env persists. Delete it before retrying: `ce.delete()` then `plugin.create_code_env()`.
...
| MXNet + NumPy 2 conflict | MXNet doesn't support NumPy 2, will downgrade to 1.26. Pin `numpy<2` if using both |
