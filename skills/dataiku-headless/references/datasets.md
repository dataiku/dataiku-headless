---
name: datasets
description: Understand and inspect Dataiku datasets, including their storage, schema, metadata, and quality signals. Use when an agent must inspect existing datasets, prepare a grounded Cobuild request, write a local CSV export in stdio, or create a new Uploaded Files dataset from user-supplied data.
---
...
5. In stdio, use `export_dataset` when every row is needed for parity checks, offline inspection, or downstream local tooling. It is unavailable in Streamable HTTP because it writes a local CSV. On large datasets this can take a while to run, so prefer the lighter inspection tools unless a full local CSV is truly needed. Use `limit` to bound the export; the tool will not export more than 1,000,000 rows in one call.
...
- In Streamable HTTP, supply direct-upload data as `columns` and at most 10,000
  `rows`; host-local file paths are stdio only.
