...
- Classify the workbook as a plain data table, analysis workbook, model workbook, report snapshot, or mixed workbook. Inventory mixed workbooks by logical object, not sheet count.
...
A report snapshot is a generated report: static values written by an upstream process (SAS, BI export, another tool's report writer), no formulas or queries, true inputs absent from the bundle. Migrating the cells would ship a dead copy; rebuild the producing logic instead.
