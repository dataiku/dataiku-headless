...
- `charts`: only series the workbook plotted or an SME plainly needs; empty is valid. Types: `line`, `bar`, `stacked_bar` (requires `series` column), `horizontal_bar`. `first`/`last` KPIs and chart ordering follow dataset row order, so point them at datasets whose order is the delivery contract.
