...
- If Cobuild cannot perform the request and no direct tool covers it, stop and report the gap. Include the DSS version when known, but attribute the gap to that version only when the requirement is established; otherwise, do not guess or fall back to raw APIs.
