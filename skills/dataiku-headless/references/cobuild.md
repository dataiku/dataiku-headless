...
- When Cobuild refuses a request, returns manual setup instructions, or reports that an asset type is unavailable, check the active instance's `dss_version` from `get_current_instance` before calling it a Headless limitation. Report an absent feature as "not available on DSS `<dss_version>`; it needs DSS `<version>`" when the feature postdates the instance, and as "not supported by Headless" only when the instance has the feature but no tool or Cobuild path reaches it. Do not silently substitute a different asset type for a version-gated one.
