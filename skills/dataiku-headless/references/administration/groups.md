---
name: groups
description: List and administer Dataiku instance groups, external mappings, and global permissions.
---
...
- Use raw Dataiku source types. SSO-backed groups use `LOCAL_NO_AUTH` and `sso_group_names`.
...
- `ldap_group_names`, `azure_ad_group_names`, `sso_group_names`, and `custom_group_names` contain the external groups that map to the Dataiku group.
...
In the Dataiku UI, these implied child permissions may appear enabled even when their stored values remain `false`.
