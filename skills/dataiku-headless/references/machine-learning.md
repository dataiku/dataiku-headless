...
6. If the request only changes where an ML task trains, use `./container-execution.md`. To place a saved-model retrain, set the existing training recipe's `params.containerSelection` through that recipe path.
