...
These are project-wide defaults, so every inheriting object moves with them. To place one specific recipe, ML task, saved-model retrain, WebApp backend, Knowledge Bank, or agent tool, use `../container-execution.md` instead.
