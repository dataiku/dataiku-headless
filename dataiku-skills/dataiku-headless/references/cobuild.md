...
- If a send is cancelled or interrupted before its response arrives, the accepted turn keeps running and stays discoverable: `list_cobuild_conversations` exposes each conversation's `last_turn_id` and `last_turn_status`. Recover by polling that exact `last_turn_id` with `get_cobuild_turn_status`; never re-send the instruction blindly. Omitting `turn_id` polls whatever the conversation's latest turn is at that moment, which may be a newer turn than the one listed, so treat it as a convenience and always compare the `turn_id` carried in the result against the id you recovered.
- Settled turns are retained for a bounded time (about an hour, up to 64 settled turns), with two exceptions that are never aged or crowded out: an unanswered deletion proposal (kept until it is answered or cancelled) and a conversation's latest turn whose result has never been delivered to any client (kept until it is observed through a poll, so an interrupted send is always recoverable). An unknown `turn_id` therefore means a restart or that its bounded retention expired; check `list_cobuild_conversations` before re-sending anything.
- The server retains up to 128 conversations, enforced before any server-side conversation is created. When full, the oldest evictable conversation is evicted to admit a new one; a conversation with a turn in flight, an unanswered deletion proposal, or a latest turn whose result has never been delivered to any client is never evicted, so an unobserved mutation always stays recoverable. If nothing is evictable, `start_cobuild_conversation` refuses with a clear error and creates nothing in DSS.
- Oversized results are bounded to 512 KB and come back as `status=error` with `error_kind=response_too_large` and `original_bytes`. An ordinary response is clipped: its `message` holds the start of the original text, with `truncated=true` and a `truncation_note`. A deletion proposal too large to inspect is never armed for approval: the server answers CANCEL to DSS and reports `cancelled_confirmation=true`. If that DSS cancel fails, the response instead carries `confirmation_disarmed_locally=true`, `cancel_failed=true`, and `cancel_error`: the id is rejected by this server, but the conversation may still await confirmation server-side. If the CANCEL itself raises a new, smaller confirmation request, that successor comes back as a first-class `status=needs_confirmation` result (its own `confirmation_id`, `objects_to_delete`, `deletion_impacts`, plus `superseded_oversized_proposal=true`), armed and inspectable and answerable by its exact id just like any deletion proposal. Either way, narrow the requested deletion and ask Cobuild to propose it again.
- The server also bounds the SDK conversation's own retained message history: after each turn, any retained message whose true payload exceeds the 512 KB terminal bound has its bulky content (the SDK's server payload, not just its short summary) cleared in place, so repeated oversized turns on one conversation do not grow memory without bound. This is transparent to the tool responses.
...
7. If a send was cancelled or interrupted and no `turn_id` was received, recover before anything else: call `list_cobuild_conversations`, read the conversation's `last_turn_id` and `last_turn_status`, then poll that exact `last_turn_id` with `get_cobuild_turn_status`. The omitted-`turn_id` form polls the conversation's current latest turn as a convenience; a newer turn may have started since the listing, so always compare the `turn_id` in the result against the one you recovered. Only re-send once the recovered turn's real outcome shows the instruction was not accepted.
...
| Poll a turn by its exact `turn_id` (omitting it polls the current latest turn) | `get_cobuild_turn_status` |
...
- After a cancelled or interrupted send, always list conversations and poll the exact recovered `last_turn_id` before ever re-sending. The accepted turn may have run (or still be running) even though its response was never delivered.
