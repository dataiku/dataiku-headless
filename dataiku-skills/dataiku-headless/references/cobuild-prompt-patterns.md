...
delegation in `../playbooks/build-via-cobuild.md`.
...
added — a proof-carrying report, not "done". You still verify independently, but a
well-shaped report tells you exactly what to verify.
...
- One verifiable unit per prompt; don't batch dependent stages that need separate
  checks.
