...
Creating a new Uploaded Files dataset is the direct-write exception (rule 2). Use it
...
2. Create with `create_upload_dataset` (a local file path visible to the server) or
   `create_upload_dataset_from_rows` (ordered columns + positional rows; tabular
   only, not arbitrary binaries). Keep overwrite off unless the user authorized
   replacement.
