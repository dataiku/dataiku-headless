...
| Dataset (managed / external) | Tabular data; type + connection decide where data lives and who owns its lifecycle | `get_dataset_info`, `get_dataset_sample`, `get_dataset_profile`, `get_dataset_metrics`, `get_dataset_column_descriptions` | Cobuild |
| Dataset (Uploaded Files) | User-supplied files/rows entering the project | same as above | Bootstrap (`create_upload_dataset`/`_from_rows`), then Cobuild |
...
| Project library | Per-project source tree (Python/R/SQL/fixtures); may include git-imported external libs | `list_project_library`, `read_project_library_file`, `search_project_library` | Cobuild (single file write → Bootstrap `write_project_library_file`) |
| Code environment | Language runtime + dependency set for code work | `list_code_envs` | Read-only (select; changes via Cobuild) |
| ML analysis | Training/experimentation config: task, features, algorithms, validation | `list_ml_analyses`, `get_ml_analysis_summary`, `get_ml_analysis_settings` | Cobuild |
| Trained model | A candidate result inside an analysis | `list_ml_analysis_models`, `get_ml_model_details` | Cobuild |
| Saved model | Deployed, versioned model artifact | `list_saved_models`, `list_saved_model_versions`, `get_saved_model_version_details` | Cobuild |
| LLM | A configured model with specific purposes (completion, embedding, rerank, image) | `list_llms`, `get_llm_info` | Read-only (referenced in prompts) |
| Knowledge Bank | Indexed content for retrieval; config sets embedding + vector-store behavior | `list_knowledge_banks`, `get_knowledge_bank_settings`, `search_knowledge_bank` | Cobuild |
| Retrieval-Augmented LLM | LLM + Knowledge Bank + retrieval settings | `list_retrieval_augmented_llms`, `get_retrieval_augmented_llm_settings` | Cobuild |
| Agent | LLM-driven agent; `agent_type` is TOOLS_USING / STRUCTURED / PYTHON | `list_agents`, `get_agent_settings`, `list_agent_versions` | Cobuild |
| Agent tool | Project-level tool an agent calls, referenced by stable id | `list_agent_tools`, `get_agent_tool_settings` | Cobuild |
| Agent review | Evaluates one agent vs test queries and named traits, over runs | `list_agent_reviews`, `get_agent_review`, `list_agent_review_tests`, `list_agent_review_runs`, `get_agent_review_run_results` | Cobuild |
| Semantic model | Business meaning over data for NL→query; container of versions, one active | `list_semantic_models`, `get_semantic_model_version_settings` | Cobuild |
| Data quality rule | Dataset-level check; outcomes OK/WARNING/ERROR/EMPTY | `list_data_quality_rules`, `get_data_quality_status`, `get_data_quality_rule`, `get_data_quality_rule_results`, `get_data_quality_rule_history` | Cobuild |
| Wiki article | Markdown doc in a parent/child tree; one home article | `list_wiki_articles`, `get_wiki_article` | Cobuild |
| Dashboard | Owns pages, layout, page filters, tiles; not the underlying insight content | `list_dashboards`, `get_dashboard_settings` | Cobuild |
| Insight | Reusable view bound to a source object (by `insightId`), a dashboard building block | `list_insights`, `get_insight_settings` | Cobuild |
| WebApp | Interactive app (STANDARD/DASH/BOKEH/SHINY/STREAMLIT); settings ≠ backend state | `list_webapps`, `get_webapp_settings`, `get_webapp_state` | Cobuild |
...
- **A WebApp's backend state is separate from its settings.** Diagnose availability
  with `get_webapp_state`; don't infer framework or backend needs from the name.
- **Wiki references are typed:** `[[Article Name]]` links an article; an object
  ...
  `scenario:`, `dashboard:`. Discover each id via its read tool; never invent one.
