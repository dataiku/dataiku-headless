...
| Scenario | `list_scenarios`, `get_scenario_settings`, `get_scenario_run_history`; lifecycle facts in `objects/scenarios.md` | Cobuild; execute an existing scenario with `run_scenario` |
...
| Data Quality rule | `list_data_quality_rules`, `get_data_quality_status`; types in `objects/data-quality-rule-types.md` | Cobuild |
...
- WebApp settings are static configuration; a running backend is a separate
