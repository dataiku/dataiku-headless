...
**The read surface is deliberately shallow.** Many object types have no direct
deep-read tool — the MCP surface gives you `list_*` discovery and a handful of
deep reads for the objects a supervisor inspects most (projects, datasets,
recipes, flow, jobs, scenarios, connections, folders, DQ). For everything else you
inspect internals by asking Cobuild in a **read-only turn**
(`send_cobuild_message` with `allow_edit_project=false`), or by reading the flow
artifacts an object produces (metrics, evaluation stores, scored outputs). That is
the design, not a gap: the deep object APIs live inside Cobuild, not here. Rows
below say `no direct deep-read` where that is the case.
...
| ML analysis | Training/experimentation config: task, features, algorithms, validation | `list_ml_analyses` (discovery); no direct deep-read — inspect one analysis via a read-only Cobuild turn | Cobuild |
...
| Saved model | Deployed, versioned model artifact | `list_saved_models` (discovery); version detail has no direct read — inspect via a read-only Cobuild turn | Cobuild |
| LLM | A configured model with specific purposes (completion, embedding, rerank, image) | `list_llms` (discovery); no direct deep-read — inspect via a read-only Cobuild turn | Read-only (referenced in prompts) |
| Knowledge Bank | Indexed content for retrieval; config sets embedding + vector-store behavior | no direct read — inspect via a read-only Cobuild turn | Cobuild |
| Retrieval-Augmented LLM | LLM + Knowledge Bank + retrieval settings | no direct read — inspect via a read-only Cobuild turn | Cobuild |
| Agent | LLM-driven agent; `agent_type` is `TOOLS_USING_AGENT` / `STRUCTURED_AGENT` / `PYTHON_AGENT` | `list_agents` (discovery); settings and versions have no direct read — inspect via a read-only Cobuild turn | Cobuild |
| Agent tool | Project-level tool an agent calls, referenced by stable id | no direct read — inspect via a read-only Cobuild turn | Cobuild |
| Agent review | Evaluates one agent vs test queries and named traits, over runs | no direct read — inspect via a read-only Cobuild turn | Cobuild |
| Semantic model | Business meaning over data for NL→query; container of versions, one active | no direct read — inspect via a read-only Cobuild turn | Cobuild |
...
| Wiki article | Markdown doc in a parent/child tree; one home article | no direct read — inspect via a read-only Cobuild turn | Cobuild |
| Dashboard | Owns pages, layout, page filters, tiles; not the underlying insight content | no direct read — inspect via a read-only Cobuild turn | Cobuild |
| Insight | Reusable view bound to a source object (by `insightId`), a dashboard building block | no direct read — inspect via a read-only Cobuild turn | Cobuild |
| WebApp | Interactive app (STANDARD/DASH/BOKEH/SHINY/STREAMLIT); settings ≠ backend state | no direct read — inspect via a read-only Cobuild turn | Cobuild |
...
- **A WebApp's backend state is separate from its settings.** There is no direct
  WebApp read here — diagnose availability via a read-only Cobuild turn; don't infer
  framework or backend needs from the name.
