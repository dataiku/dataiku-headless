...
Source Subskill = directory `dataiku-skills/dataiku-headless/references/migration-sources/<kind>/` (kind kebab-case): reference `<kind>.md` plus optional helper scripts `*.py`.
...
- Validation catches wrong SHAPE — schema / data-invariant (`../../playbooks/migrate.md`, Phase 3).
- Planning catches unbuildable constructs (`../../playbooks/migrate.md`, Phase 1).
...
DROP everything else: obvious 1:1 tool→recipe mappings, ANY mapping / translation table, ALL code / CLI / XML / GREL example blocks, rationale / "why" prose, collapse or optimization hints, unbuildable / equivalent-lacking constructs (planning's job).
...
Carry only source-specific knowledge. Reference `../../playbooks/migrate.md` for the workflow and plans.

Estimated size 30–90 lines.

## Helper scripts (optional)

Atomic source-parsers in `<kind>/*.py` beside the reference, one purpose each, named by purpose; directory carries the kind. Author as generic starting points the planner adapts. Declare deps minimally, prefer stdlib plus one parsing library. Output, stdout-digest, and first-docstring-line conventions: `../../playbooks/migrate.md`, "Source parsing helpers".

## Routing

Runtime routing lists the known sources in `../../playbooks/migrate.md` (Phase 1) — add the new kind there; shipping = one PR adding one `migration-sources/<kind>/` directory: the `<kind>.md` reference plus any helper scripts.
