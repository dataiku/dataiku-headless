...
1. Inspect the default response for license validity, expiration, and profile capacity.
2. Use `direct_count` for users directly assigned to a profile. Compare `count_with_demoted_to` to `licensed_limit` to understand if a profile type can be assigned to any additional users.
