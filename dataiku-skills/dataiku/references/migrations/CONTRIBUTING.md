...
Source Subskill = directory `dataiku-skills/migrations/sources/<kind>/` (kind kebab-case): reference `<kind>.md` plus optional helper scripts `*.py`.
...
- Gap resolution catches unbuildable constructs (parent `SKILL.md` § Gap resolution).
...
Carry only source-specific knowledge. Reference parent `SKILL.md` for workflow, Manifest, gap resolution.

Estimated size 30–90 lines.

## Helper scripts (optional)

Atomic source-parsers in `sources/<kind>/*.py`, one purpose each, named by purpose; directory carries the kind. Author as generic starting points the planner adapts. Declare deps minimally, prefer stdlib plus one parsing library. Output, stdout-digest, and first-docstring-line conventions: parent `SKILL.md` § Helper scripts.

## Routing

Runtime routing lists `dataiku-skills/migrations/sources/` (parent `SKILL.md` Phase 2); shipping = one PR adding one `sources/<kind>/` directory: the `<kind>.md` reference plus any helper scripts.
