...
diffs it; `tests/test_skill_links.py` runs the tool-name checker). `release.yml` additionally runs
`scripts/check_skill_tool_names.py` standalone. To exercise the server end to end:
...
`CHANGELOG.md`. Full workflow, PR checklist, and release steps are in **`CONTRIBUTING.md`**.
...
| `CONTRIBUTING.md` | Workflow, Conventional Commits, PR checklist, release, security reporting |
