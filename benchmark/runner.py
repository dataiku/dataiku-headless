...
from benchmark.agents.base import AgentResult, BaseAgent, VerificationCheck, VerificationResult
from benchmark.agents.claude import ClaudeAgent
from benchmark.agents.codex import CodexAgent
from benchmark.analyzer.recommender import Recommender
from benchmark.analyzer.reporter import Reporter, TestResultRecord
from benchmark.analyzer.scorer import Score, Scorer
from benchmark.analyzer.trace_parser import summarize_trace
from benchmark.scenarios.schema import Scenario, load_scenarios
...
def main():
    ...
    parser.add_argument("--agent", type=str, help="Agent to test (claude, codex, or comma-separated)")
    ...
    parser.add_argument("--parallel", type=int, default=None, help="Max parallel workers")
    parser.add_argument("--config", type=str, default=None, help="Config file path")
    parser.add_argument("--dry-run", action="store_true", help="Show what would run without executing")
    parser.add_argument("--skip-preflight", action="store_true", help="Skip DSS connectivity check")
    ...
    if args.dry_run:
        ...
        for s in scenarios:
            for a in agents:
                print(f"    [{a.name:6s}] {s.id:25s} (tier {s.tier}, project={'yes' if s.needs_project else 'no'})")
        return
    ...
    with ThreadPoolExecutor(max_workers=parallel) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"  [{completed:3d}/{total_runs}] {status} {agent_name:6s} {test_id:25s} "
                      f"score={score:.2f} {duration}ms")
            except Exception as e:
                completed += 1
                print(f"  [{completed:3d}/{total_runs}] ERR  {agent_name:6s} {test_id:25s} {e}")
