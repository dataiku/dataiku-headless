...
def test_wiki_scan_reports_when_its_heading_evidence_is_bounded():
    proj = linear_project()
    proj._wiki = [
        (f"Article {i}", "# Purpose\nText\n# Inputs\nSource")
        for i in range(ae.MAX_WIKI_ARTICLES + 1)
    ]

    checks = checks_for_bucket(proj, "documentation")

    assert checks["wiki_scan_complete"].status == "warn"
    assert "bounded" in checks["wiki_scan_complete"].detail
...
def test_denied_terminal_metadata_read_is_advisory_but_visible():
    ...
    assert checks["flow_visible_descriptions"].status == "warn"
