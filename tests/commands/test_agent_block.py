...
def test_list_blocks(patch_client):
    result = runner.invoke(app, ["agent-block", "list", "agent_blocks", "--project", "PROJ1"])
...
def test_list_blocks_json(patch_client):
    result = runner.invoke(app, ["agent-block", "list", "agent_blocks", "--project", "PROJ1", "-o", "json"])
    assert result.exit_code == 0
...
def test_get_block(patch_client):
    result = runner.invoke(app, ["agent-block", "get", "agent_blocks", "classify", "--project", "PROJ1"])
    assert result.exit_code == 0
    assert "classify" in result.output
...
def test_get_block_json(patch_client):
    result = runner.invoke(app, ["agent-block", "get", "agent_blocks", "classify", "--project", "PROJ1", "-o", "json"])
    assert result.exit_code == 0
    parsed = json.loads(result.output)
...
def test_get_block_not_found(patch_client):
    result = runner.invoke(app, ["agent-block", "get", "agent_blocks", "nonexistent", "--project", "PROJ1"])
    assert result.exit_code != 0
...
def test_add_block(patch_client):
    ...
    result = runner.invoke(app, ["agent-block", "add", "agent_blocks", "--block", block, "--project", "PROJ1"])
    assert result.exit_code == 0
    assert "new_emit" in result.output

    # Verify block was appended
    settings = patch_client.get_project("PROJ1").get_agent("agent_blocks").get_settings()
...
def test_add_block_set_start(patch_client):
    ...
    result = runner.invoke(app, [
        "agent-block", "add", "agent_blocks", "--block", block, "--set-start", "--project", "PROJ1",
    ])
    assert result.exit_code == 0

    raw = patch_client.get_project("PROJ1").get_agent("agent_blocks").get_settings().get_raw()
    assert raw["versions"][0]["toolsUsingAgentSettings"]["startingBlockId"] == "new_start"
...
def test_add_block_auto_mode_switch(patch_client):
    ...
    block = json.dumps({"type": "EMIT_OUTPUT", "id": "first_block", "template": "Hello"})
    result = runner.invoke(app, ["agent-block", "add", "agent1", "--block", block, "--project", "PROJ1"])
    assert result.exit_code == 0
...
def test_remove_block(patch_client):
    result = runner.invoke(app, ["agent-block", "remove", "agent_blocks", "emit_result", "--project", "PROJ1"])
    assert result.exit_code == 0
...
def test_remove_block_not_found(patch_client):
    result = runner.invoke(app, ["agent-block", "remove", "agent_blocks", "nonexistent", "--project", "PROJ1"])
    assert result.exit_code != 0


def test_remove_starting_block(patch_client):
    """Removing the starting block should clear startingBlockId."""
    result = runner.invoke(app, ["agent-block", "remove", "agent_blocks", "init_state", "--project", "PROJ1"])
    assert result.exit_code == 0
...
def test_connect_blocks(patch_client):
    result = runner.invoke(app, [
        "agent-block", "connect", "agent_blocks",
        "--from", "emit_result", "--to", "init_state",
        "--project", "PROJ1",
    ])
    assert result.exit_code == 0
...
def test_disconnect_block(patch_client):
    result = runner.invoke(app, ["agent-block", "disconnect", "agent_blocks", "classify", "--project", "PROJ1"])
    assert result.exit_code == 0
...
def test_disconnect_not_found(patch_client):
    result = runner.invoke(app, ["agent-block", "disconnect", "agent_blocks", "nonexistent", "--project", "PROJ1"])
    assert result.exit_code != 0
...
def test_set_start(patch_client):
    result = runner.invoke(app, ["agent-block", "set-start", "agent_blocks", "classify", "--project", "PROJ1"])
    assert result.exit_code == 0

    raw = patch_client.get_project("PROJ1").get_agent("agent_blocks").get_settings().get_raw()
    assert raw["versions"][0]["toolsUsingAgentSettings"]["startingBlockId"] == "classify"


def test_set_start_not_found(patch_client):
    result = runner.invoke(app, ["agent-block", "set-start", "agent_blocks", "nonexistent", "--project", "PROJ1"])
    assert result.exit_code != 0
...
def test_set_mode_to_blocks_graph(patch_client):
    result = runner.invoke(app, ["agent-block", "set-mode", "agent1", "BLOCKS_GRAPH", "--project", "PROJ1"])
    assert result.exit_code == 0
...
def test_set_mode_to_simple(patch_client):
    result = runner.invoke(app, ["agent-block", "set-mode", "agent_blocks", "SIMPLE", "--project", "PROJ1"])
    assert result.exit_code == 0
...
def test_set_mode_invalid(patch_client):
    result = runner.invoke(app, ["agent-block", "set-mode", "agent_blocks", "INVALID", "--project", "PROJ1"])
    assert result.exit_code != 0
...
def test_get_graph(patch_client):
    result = runner.invoke(app, ["agent-block", "get-graph", "agent_blocks", "--project", "PROJ1"])
    assert result.exit_code == 0
    parsed = json.loads(result.output)
...
def test_set_graph(patch_client):
    new_graph = json.dumps({
        ...
        "blocks": [
            {"type": "EMIT_OUTPUT", "id": "greet", "template": "Hi", "addToMessages": True},
        ],
        "tools": [],
    })
    result = runner.invoke(app, [
        "agent-block", "set-graph", "agent_blocks", "--definition", new_graph, "--project", "PROJ1",
    ])
    assert result.exit_code == 0
...
def test_set_graph_from_file(patch_client, tmp_path):
    ...
    result = runner.invoke(app, [
        "agent-block", "set-graph", "agent_blocks", "--definition", f"@{graph_file}", "--project", "PROJ1",
    ])
    assert result.exit_code == 0
...
def test_list_blocks_agent_not_found(patch_client):
    result = runner.invoke(app, ["agent-block", "list", "nonexistent_agent", "--project", "PROJ1"])
    assert result.exit_code != 0
