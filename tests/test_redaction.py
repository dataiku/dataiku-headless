"""Contract tests for shared, structure-aware secret redaction."""
...
def test_inline_secret_param_masks_value_but_keeps_reviewable_metadata():
    entry = {
        "key": "api_key",
    ...
    assert redact_sensitive_values(entry) == {
        "key": "api_key",
        "value": VARIABLE_REDACTION,
        "secret": True,
        "type": "STRING",
    }


def test_typed_password_param_is_masked_and_non_secret_param_survives():
    assert redact_sensitive_values(
        {"name": "token", "value": "topsecret", "type": "PASSWORD"}
    ) == {"name": "token", "value": VARIABLE_REDACTION, "type": "PASSWORD"}
    ordinary = {"key": "temperature", "value": "0.2", "secret": False}
