"""Mock DSS client factory used by CLI command tests."""
...
def create_mock_client():
    ...
    _at_local = [
        ...
        # IDs referenced by agent add-tool tests — the command validates tool
        # refs against list_agent_tools before writing toolRef.
        {"id": "existing_tool", "name": "Existing Tool", "type": "DatasetRowLookup"},
        {"id": "new_tool", "name": "New Tool", "type": "DatasetRowLookup"},
        {"id": "fresh_tool", "name": "Fresh Tool", "type": "DatasetRowLookup"},
        {"id": "new_tool_2", "name": "New Tool 2", "type": "DatasetRowLookup"},
    ]
    _at_shared = [
        {
            "id": "shared_tool",
    ...
    # Analysis definition + training-recipe input agree by default so the
    # redeploy stale-input advisory stays quiet unless a test changes one.
    analysis_mock = MagicMock()
    analysis_mock.get_definition.return_value.get_raw.return_value = {
        "inputDatasetSmartName": "train_ds"
    }
    proj1.get_analysis.return_value = analysis_mock
    recipe_settings.get_recipe_inputs.return_value = {
        "main": {"items": [{"ref": "train_ds", "deps": []}]}
    }
    ...
    # App Designer — project-level manifest used by dku app-designer commands
    app_manifest_raw = {
        "useAppHomepage": True,
        "label": "My Test App",
    ...
    app_manifest_mock_proj = MagicMock()
    app_manifest_mock_proj.get_raw.return_value = app_manifest_raw
    app_manifest_mock_proj.raw_data = app_manifest_raw
    app_manifest_mock_proj.project_key = "PROJ1"
    app_manifest_mock_proj.save.return_value = None
    proj1.get_app_manifest.return_value = app_manifest_mock_proj
    ...
    configure_govern_client(client)
