...
version-discovery payloads, field-name secret redaction (the one shared
redactor), and the plain size-check-and-clip byte ceiling.
"""
...
# --------------------------------------------------------------------------- #
# Bounding — plain size-check-and-clip (no binary search)
# --------------------------------------------------------------------------- #
...
def test_quote_heavy_payload_is_clipped_to_the_settings_ceiling(monkeypatch):
    # A quote-heavy payload (WebApp code) roughly doubles when escaped into the
    # outer JSON. The v1 helper is a plain size-check-and-clip, not a binary
    # search, so it bounds the SETTINGS blob to the ceiling (returned_bytes) and
    # keeps truthful counts — it does not guarantee a byte-exact final frame.
    payload = {"code": '"' * (object_settings.MAX_SETTINGS_BYTES + 200_000)}
    bind(monkeypatch, FakeProject(webapps={"w": payload}))
    out = call(object_type="webapp", object_id="w")
    assert out["truncated"] is True
    assert out["returned_bytes"] <= object_settings.MAX_SETTINGS_BYTES
...
def test_dss_client_is_captured_before_first_await(monkeypatch):
    first_project = FakeProject(dashboards={"d": {"source": "first"}})
    second_project = FakeProject(dashboards={"d": {"source": "second"}})
    clients = [FakeClient(first_project), FakeClient(second_project)]
    calls = []
    ...
    monkeypatch.setattr(object_settings, "get_dss_client", current_client)

    assert call(object_type="dashboard", object_id="d")["settings"] == {
        "source": "first"
    }
    assert len(calls) == 1


def test_non_string_version_id_is_rejected(monkeypatch):
    bind(monkeypatch, FakeProject(agents={"a": {"versions": []}}))
    with pytest.raises(ValueError, match="version_id.*string"):
        call(object_type="agent", object_id="a", version_id=123)


def test_backend_exception_text_is_not_returned(monkeypatch):
    class LeakyProject(FakeProject):
        def get_dashboard(self, object_id):
            raise DataikuException("password=hunter2")

    bind(monkeypatch, LeakyProject())

    with pytest.raises(ValueError) as exc:
        call(object_type="dashboard", object_id="d")

    assert "hunter2" not in str(exc.value)
    assert "DataikuException" in str(exc.value)
