...
@pytest.mark.parametrize("field", ["project_key", "object_id", "version_id"])
def test_identifiers_cannot_bypass_final_response_ceiling(monkeypatch, field):
    bind(monkeypatch, FakeProject(dashboards={"d": {}}))
    kwargs = {"object_type": "dashboard", "object_id": "d"}
    if field == "version_id":
        kwargs["object_type"] = "agent"
        kwargs["object_id"] = "a"
    kwargs[field] = "x" * (object_settings.MAX_IDENTIFIER_CHARS + 1)
