...
def test_overview_clips_oversized_variable_values(monkeypatch):
    project = FakeProject(
        "PROJ",
    ...
    # Oversized top-level string values are clipped to a bounded length; the
    # simple clip does not raise a warning (no deep depth/node-count walker).
    assert len(res["variables"]["huge"]) == projects._MAX_TEXT_CHARS
    assert res["variables"]["huge"].endswith("…")
    assert "warnings" not in res
