...
def invoke(self, input, trace):
    ...
    if trace:
        trace.add_metadata("tool_version", "1.0")
        trace.add_metadata("parameters", input)
...
| `'SpanBuilder' has no attribute 'set_attribute'` | Use `trace.add_metadata(key, value)` not `trace.set_attribute()` |
...
The `dataiku` module imports numpy/pandas/dateutil at load time, so they're required even if your tool doesn't use them.
