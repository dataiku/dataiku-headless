...
def resolve_dotenv_path() -> Path:
    ...
    for candidate in candidates:
        if candidate.is_file():
            return candidate
    # Nothing on disk: default to the cwd-relative ``./.env`` (a missing file is
    # a no-op for ``load_dotenv``), never the DKU_CONFIG_DIR or XDG candidate.
    return Path.cwd() / ".env"
...
def switch_instance(name: str) -> dict:
    ...
    The switch is atomic with respect to concurrent readers. A caller that has
    already captured a ``DSSInstance`` snapshot and client keeps using them;
    callers that resolve the active instance later observe the new selection.
    """
    with _registry_lock:
