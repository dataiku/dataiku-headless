...
@mcp.tool()
async def create_project_library_folder(
    project_key: str,
    path: str,
    ctx: Context,
) -> str:
    """Create a folder in the project library, including any missing parents."""
    project_key = _require_non_empty_string(project_key, "project_key")
    path = _normalize_library_path(path)
    if path == "/":
        raise ValueError("path cannot be '/'")
    await ctx.info(f"Creating project library folder {path} in {project_key}...")
    ...
    def _run():
        ...
        for component in parts:
            ...
            if existing is None:
                ...
                raise ValueError(
                    f"Path component '{component}' is a file, not a folder"
                )
            else:
                folder = existing
        return {
...
@mcp.tool()
async def rename_project_library_item(
    project_key: str,
    path: str,
    new_name: str,
    ctx: Context,
    ...
    project_key = _require_non_empty_string(project_key, "project_key")
    path = _normalize_library_path(path)
    new_name = _require_non_empty_string(new_name, "new_name")
    if "/" in new_name:
        raise ValueError(
            "new_name must not contain '/'. Use move_project_library_item to relocate."
        )
    await ctx.info(f"Renaming project library item {path} -> {new_name} in {project_key}...")

    def _run():
        library = get_dss_client().get_project(project_key).get_library()
        item = _resolve_item(library, path)
        if item is None:
...
@mcp.tool()
async def move_project_library_item(
    project_key: str,
    path: str,
    destination_folder_path: str,
    ctx: Context,
) -> str:
    """Move a file or folder under another folder."""
    project_key = _require_non_empty_string(project_key, "project_key")
    path = _normalize_library_path(path)
    destination_folder_path = _normalize_library_path(destination_folder_path)
    await ctx.info(
        f"Moving {path} to {destination_folder_path} in {project_key}..."
    )
    ...
    def _run():
        ...
        destination = (
            ...
            else _safe_get_folder(library, destination_folder_path)
        )
...
@mcp.tool()
async def delete_project_library_item(
    project_key: str,
    path: str,
    ctx: Context,
    recursive: bool = False,
) -> str:
    """Delete a file or folder from the project library."""
    project_key = _require_non_empty_string(project_key, "project_key")
    path = _normalize_library_path(path)
    await ctx.info(f"Deleting project library item {path} in {project_key}...")
    ...
    def _run():
        ...
        if folder is not None:
            if not recursive:
                raise ValueError("Path is a folder. Pass recursive=True to delete it.")
            folder.delete()
            return {
                "path": path,
                "type": "folder",
...
@mcp.tool()
async def set_external_library(
    project_key: str,
    local_path: str,
    ctx: Context,
    remote: str = "",
    ...
    """Create, update, or remove a git-imported external library."""
    project_key = _require_non_empty_string(project_key, "project_key")
    local_path = local_path.strip().strip("/")
    if not local_path:
        raise ValueError("local_path must be a non-empty string")
    if enabled:
        remote = _require_non_empty_string(remote, "remote")
        checkout = _require_non_empty_string(checkout, "checkout")
        await ctx.info(
            f"Configuring external library {local_path} from {remote}@{checkout} "
            f"in {project_key}..."
        )
    else:
        await ctx.info(f"Removing external library {local_path} from {project_key}...")
    ...
    def _run():
        ...
        if external_error is not None:
            raise ValueError(
                "External library configuration is unavailable, so this mutation "
                f"cannot safely determine current state. Details: {external_error}"
            )
        existing = {
            entry["local_path"]: entry for entry in external_libraries
        }.get(local_path)
        ...
        if not enabled:
            ...
            project_git.remove_library(local_path, False)
            return {
                "local_path": local_path,
                "action": "removed",
            }
        ...
        if existing is None:
            future = project_git.add_library(
                repository=remote,
                local_target_path=local_path,
                checkout=checkout,
                path_in_git_repository=path_in_repo,
        ...
        current = {
            ...
            "path_in_repo": existing["path_in_repo"],
        }
        if current == desired:
            return {
                "local_path": local_path,
        ...
        project_git.set_library(
            local_path,
            remote=remote,
            remotePath=path_in_repo,
            checkout=checkout,
        )
        project_git.reset_library(local_path).wait_for_result()
        return {
            "local_path": local_path,
            "action": "updated",
        }
