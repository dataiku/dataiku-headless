...
``get_object_settings`` coexists with the narrow readers in this review slice.
That makes equivalence testable before a later PR removes only the specialist
settings tools it truly subsumes. It dispatches through a closed ``object_type``
enum to the same ``dataikuapi`` accessors and returns the same evidence: usually
the raw settings dictionary, with purpose-shaped envelopes for wiki articles,
ML analyses, model versions, and evaluation stores.

Runtime evidence is deliberately outside the equivalence claim. A WebApp's live
backend state and an agent review's runs/results still require their dedicated
read tools. Datasets, recipes, scenarios, connections, folders, jobs, and project
metadata also retain their dedicated deep reads.

Every result is recursively redacted (including inline DSS secret parameters)
and the final serialized response has a hard byte ceiling.
"""
...
def _bounded_identifier(value: str, field_name: str) -> str:
    ...
    if len(cleaned) > MAX_IDENTIFIER_CHARS:
        raise ValueError(
            f"'{field_name}' must be <= {MAX_IDENTIFIER_CHARS} characters"
        )
    return cleaned
...
async def get_object_settings(
    ...
    bounded to a hard byte ceiling. Use the dedicated runtime read tool when
    ...
    """
    project_key = _bounded_identifier(project_key, "project_key")
    object_type = _require_allowed_value(
        _require_non_empty_string(object_type, "object_type").lower(),
        "object_type",
        _OBJECT_TYPES,
    )
    object_id = _bounded_identifier(object_id, "object_id")
    if not isinstance(version_id, str):
        raise ValueError("'version_id' must be a string")
    version_id = version_id.strip()
    if len(version_id) > MAX_IDENTIFIER_CHARS:
        raise ValueError(
            f"'version_id' must be <= {MAX_IDENTIFIER_CHARS} characters"
        )
