...
_MAX_VARIABLE_NODES = 1_000
_MAX_VARIABLE_DEPTH = 8
...
async def get_project_variables(
    ...
    warnings: list[str] = []
    result = {
        "standard": _bound_nested(
            redact_sensitive_values(variables.get("standard", {}) or {}),
            warnings,
            "standard",
        )
    }
    if include_local:
        result["local"] = _bound_nested(
            redact_sensitive_values(variables.get("local", {}) or {}),
            warnings,
            "local",
        )
    if warnings:
...
def _bound_nested(value, warnings: list[str], section: str):
    """Bound a nested variable payload by depth, node count, and string length."""
    budget = {"nodes": _MAX_VARIABLE_NODES, "truncated": False}
        ...
        if isinstance(item, dict):
            ...
            for key, nested in item.items():
                if budget["nodes"] <= 0:
                    budget["truncated"] = True
                    break
                out[_clip_text(key, 256)] = walk(nested, depth + 1)
            return out
        if isinstance(item, list):
            ...
            for nested in item:
                if budget["nodes"] <= 0:
                    budget["truncated"] = True
                    break
                out.append(walk(nested, depth + 1))
            return out
        if isinstance(item, str):
            clipped = _clip_text(item)
            budget["truncated"] |= clipped != item
            return clipped
        return item
    ...
    if budget["truncated"]:
        warnings.append(
            f"{section}: nested content was bounded for a safe response "
            f"({_MAX_VARIABLE_NODES} values, depth {_MAX_VARIABLE_DEPTH}, "
            f"{_MAX_TEXT_CHARS} characters per string)"
        )
    return bounded
...
async def get_project_overview(
    ...
    are recursively redacted and bounded. Local variables are never returned here.
    """
    project_key = _require_non_empty_string(project_key, "project_key")
    ...
    def _run():
        ...
        if variables is not None:
            variables = _bound_nested(
                redact_sensitive_values(variables or {}), warnings, "variables"
            )
