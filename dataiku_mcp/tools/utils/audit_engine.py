...
This is a slimmed, harness-independent port of the ``dku`` CLI ``project-audit``
engine. It separates two questions: can DSS read the Flow evidence and satisfy an
explicit delegated-output contract; and does the Flow carry the basic
documentation a human needs to review it? The first is a fail-closed *gate* — a
deterministic pass/fail signal, not an oracle that proves the build is correct.
The second (and the layout heuristics) are advisory, because names, zones,
descriptions, and empty graph leaves are conventions, not proof of correctness.

Everything here is read-only. Metrics are read from their *cached* values
(never recomputed — a cached count can be stale); row samples are bounded to
:data:`MAX_SAMPLE_ROWS`; the only heavy operation (the flow-consistency check) is
time-bounded. Nothing mutates DSS.

Only **FAIL-severity** checks fail closed. A FAIL check whose evidence cannot be
read (it raised or timed out) degrades to an ``error`` — unreadable evidence
blocks the gate and marks the audit ``incomplete``, never a silent pass. A
WARN-severity check that cannot run degrades to a visible ``skip``: advisory, so
it never blocks, but it is surfaced so the supervisor knows it did not run.

The verdict spans three buckets (structure, documentation, evidence) plus an
optional contract, and five statuses a :class:`Check` can carry:

* ``pass`` — the check's evidence supports its narrow assertion.
* ``fail`` — DSS consistency or an explicit contract assertion failed (blocks).
* ``warn`` — a reviewability heuristic found something to inspect (advisory).
...
Callers pass a ``dataikuapi`` project handle and compose :func:`load_context` +
:func:`run_bucket` + :func:`run_contract_checks` + :func:`build_payload` to
interleave progress reporting (as the MCP tool wrapper does, one ``run_blocking``
call per bucket).
"""
...
from .serialization import columnar, compact_json
...
MAX_SAMPLE_ROWS = 100
MAX_CONTRACT_OUTPUTS = 25
MAX_CONTRACT_FIELDS = 200
MAX_AUDIT_IDENTIFIER_CHARS = 512
MAX_RESULT_TEXT_CHARS = 2_000
MAX_INVENTORY_ITEMS = 250
MAX_AUDIT_RESPONSE_BYTES = 1_000_000
...
AUDIT_BUCKETS_ORDER = ("structure", "documentation", "evidence")
            ...
            if severity == FAIL:
                out.append(
                    _error(
                        ...
                        bucket=bucket,
                        severity=severity,
                        detail=f"check could not be evaluated ({type(exc).__name__})",
                    )
                )
            else:
                out.append(
                    _skip(
                        ...
                        bucket=bucket,
                        severity=severity,
                        detail=f"check could not run ({type(exc).__name__})",
                    )
                )
            continue
        if result is None:
...
def _terminal_columns_scan(proj, terminal: set[str]) -> tuple[list[str], list[str]]:
    ...
    as an explicit gap, never silently treated as "documented". A dataset with no
    schema at all is neither undocumented nor unreadable.
    """
...
def _terminal_row_count(proj, name: str) -> int:
    ...
    Reads the *cached* metric value (which can be stale); never recomputes it.
            ...
            for key in ("recipeCheckResult", "datasetCheckResult"):
                for msg in node_state.get(key, {}).get("messages", []):
                    if msg.get("isFatal") or msg.get("severity") in ("ERROR", "FATAL"):
                        # DSS messages can include SQL, URLs, or connection details.
                        # The node and stable message code locate the failure without
                        # echoing backend text into the model transcript.
                        code = msg.get("code") or "fatal consistency error"
                        return False, f"{node_id}: {code}"
        return True, ""
    finally:
...
# --------------------------------------------------------------------------- #
# Description scans (basic presence: does the object carry a description at all)
# --------------------------------------------------------------------------- #
# A description scan returns (problems, unreadable): the missing-description
# problems found among readable objects, and the names of objects whose metadata
# read *raised* (unreadable evidence — never silently dropped, per fail-closed
# auditing). Description *quality* is not judged; only presence.
...
def _description_scan(reader, proj, names: list[str], label: str) -> DescriptionScan:
    ...
    A basic presence check only: an object with any non-empty description passes;
    an object with none is reported as missing. Description quality is not judged.
    """
    missing: list[str] = []
    unreadable: list[str] = []
    for name in names:
        ...
        if not text:
            missing.append(name)
    problems = (
        [f"{label} without a description: {', '.join(sorted(missing))}"] if missing else []
    )
...
def _zone_dicts(proj) -> list[dict[str, Any]]:
    ...
    for z in proj.get_flow().list_zones():
        raw = z.get_settings().get_raw() or {}
        items = raw.get("items", []) or []
        zones.append(
            {
                "id": raw.get("id", getattr(z, "id", "")),
                "name": raw.get("name", getattr(z, "name", "")),
...
def _has_wiki_articles(proj) -> bool:
    """True when the project wiki has at least one article (presence check only).

    The audit never scans wiki *bodies* — it asserts a runbook exists, not that
    its prose covers any particular section.
    """
    return bool(proj.get_wiki().list_articles())
...
# --------------------------------------------------------------------------- #
# Bucket: documentation (basic presence checks only — no text-quality heuristics)
# --------------------------------------------------------------------------- #
...
def _has_wiki_check(actx: AuditContext) -> Check:
    if _has_wiki_articles(actx.proj):
...
def _documentation_units(actx: AuditContext) -> list[CheckUnit]:
    proj = actx.proj
    terminal = actx.terminal
    sources = actx.inputs - actx.outputs
...
def _terminal_outputs_built_check(sweep: dict[str, Any]) -> Check:
    """Advisory heuristic: terminal outputs usually have cached, non-zero rows.

    A graph leaf can legitimately be empty, and a missing cached metric is not
    proof of failure. This check therefore warns; callers that require a row count
    should express it as an explicit ``contract.min_rows`` assertion, which is a
    fail-closed gate.

    * a proven-empty output (count <= 0) is a real fail — report it (and note any
      outputs whose count could not be read);
    * no proven-empty output but an unreadable count becomes a visible skip;
    * only when every output's count was readable and non-empty does it pass.
    ...
    if unknown:
        return _skip(
            ...
            severity=WARN,
            detail=(
                "record-count evidence unavailable for terminal outputs: "
                + ", ".join(unknown)
            ),
        )
    return _ok("terminal_outputs_built", bucket="evidence", severity=WARN)
...
def run_bucket(actx: AuditContext, bucket: str) -> list[Check]:
    """Run a bucket; failed reads become ``error`` for gates and ``skip`` for warnings."""
...
def _validate_string_list(dataset: str, field_name: str, value: Any) -> None:
    ...
    if len(value) > MAX_CONTRACT_FIELDS:
        raise ValueError(
            f'contract output "{dataset}": "{field_name}" must contain at most '
            f"{MAX_CONTRACT_FIELDS} entries"
        )
    for i, entry in enumerate(value):
        ...
        if len(entry.strip()) > MAX_AUDIT_IDENTIFIER_CHARS:
            raise ValueError(
                f'contract output "{dataset}": "{field_name}"[{i}] must be <= '
                f"{MAX_AUDIT_IDENTIFIER_CHARS} characters"
            )
...
def _validate_types(dataset: str, value: Any) -> None:
    ...
    if len(value) > MAX_CONTRACT_FIELDS:
        raise ValueError(
            f'contract output "{dataset}": "types" must contain at most '
            f"{MAX_CONTRACT_FIELDS} entries"
        )
    for column, expected in value.items():
        ...
        if len(column.strip()) > MAX_AUDIT_IDENTIFIER_CHARS:
            raise ValueError(
                f'contract output "{dataset}": "types" keys must be <= '
                f"{MAX_AUDIT_IDENTIFIER_CHARS} characters"
            )
        ...
        if len(expected.strip()) > MAX_AUDIT_IDENTIFIER_CHARS:
            raise ValueError(
                f'contract output "{dataset}": "types" values must be <= '
                f"{MAX_AUDIT_IDENTIFIER_CHARS} characters"
            )
...
def _validate_output_spec(dataset: str, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    unknown = sorted(repr(k) for k in spec if k not in _KNOWN_OUTPUT_SPEC_KEYS)
    if unknown:
        raise ValueError(
...
def normalize_contract(contract: Any) -> dict[str, Any]:
    ...
    ``columns`` and ``types`` assert a *required subset* of the saved schema — the
    listed columns must exist / match, extra columns in the output never fail it.
    """
    if not isinstance(contract, dict):
        raise ValueError("contract must be a JSON object")
    unknown_top = sorted(repr(k) for k in contract if k not in _KNOWN_CONTRACT_KEYS)
    if unknown_top:
        raise ValueError(
    ...
    if isinstance(outputs, list):
        ...
        if len(outputs) > MAX_CONTRACT_OUTPUTS:
            raise ValueError(
                f'contract "outputs" must contain at most {MAX_CONTRACT_OUTPUTS} entries'
            )
        for i, entry in enumerate(outputs):
            ...
            if len(dataset) > MAX_AUDIT_IDENTIFIER_CHARS:
                raise ValueError(
                    f'contract outputs[{i}] "dataset" must be <= '
                    f"{MAX_AUDIT_IDENTIFIER_CHARS} characters"
                )
        ...
        for name, spec in outputs.items():
            ...
            if len(clean_name) > MAX_AUDIT_IDENTIFIER_CHARS:
                raise ValueError(
                    "contract output names must be <= "
                    f"{MAX_AUDIT_IDENTIFIER_CHARS} characters"
                )
...
def _contract_types_check(dataset: str, spec: dict[str, Any], columns: list[dict[str, Any]]) -> Check:
    # Checks only the columns the contract names, against the saved schema — a
    # required subset, not exact schema equality (extra columns never fail it).
...
def _contract_min_rows_check(proj, dataset: str, spec: dict[str, Any]) -> Check:
    ...
    # incomplete) so the shortfall verdict only ever rests on a real (cached) count.
    try:
        count = _terminal_row_count(proj, dataset)
        ...
        return _error(
            ...
            detail=(
                ...
                f"evidence unreadable ({type(exc).__name__})"
            ),
        )
...
def _contract_not_blank_check(proj, dataset: str, spec: dict[str, Any], columns: list[dict[str, Any]]) -> Check:
    # Samples at most MAX_SAMPLE_ROWS rows; an empty sample cannot certify the
    # assertion, so it errors rather than passing vacuously.
    ...
    if not rows:
        return _error(
            ...
            severity=FAIL,
            detail=(
                f"cannot verify the sampled not_blank contract for {dataset} — "
                "no rows were available to sample"
            ),
        )
...
def validate_buckets(buckets: list[str] | None) -> list[str]:
    """Validate + canonicalize the requested bucket names; default is all three."""
    if buckets is None:
...
def _clip_result_text(value: Any, limit: int = MAX_RESULT_TEXT_CHARS) -> tuple[str, bool]:
...
def _bounded_inventory(inventory: dict[str, Any]) -> dict[str, Any]:
    bounded: dict[str, Any] = {}
    truncated: dict[str, dict[str, int]] = {}
    for key, value in inventory.items():
        if isinstance(value, list):
            returned = []
            for item in value[:MAX_INVENTORY_ITEMS]:
                clipped, _ = _clip_result_text(item, 256)
                returned.append(clipped)
            bounded[key] = returned
            if len(value) > len(returned):
                truncated[key] = {"total": len(value), "returned": len(returned)}
        else:
            bounded[key] = value
    if truncated:
...
def _check_payload(check: Check) -> dict[str, Any]:
    row = asdict(check)
    detail, detail_truncated = _clip_result_text(row.get("detail"))
    fix, fix_truncated = _clip_result_text(row.get("fix"))
    row["detail"] = detail
    row["fix"] = fix
    if detail_truncated or fix_truncated:
...
def build_payload(project_key: str, checks: list[Check], inventory: dict[str, Any]) -> dict[str, Any]:
    ...
    if issues:
        checks_block["issues"] = columnar(
            [_check_payload(c) for c in issues],
            [
                "id",
                "bucket",
                "severity",
    ...
    payload: dict[str, Any] = {
        "project_key": _clip_result_text(project_key, MAX_AUDIT_IDENTIFIER_CHARS)[0],
        "scope": AUDIT_SCOPE,
        "passed": not failed and not errored,
        "score": score,
        "summary": summary,
        "checks": checks_block,
        "inventory": _bounded_inventory(inventory),
    }
    if errored:
...
def _fit_rows(reduced: dict[str, Any], container: dict[str, Any], key: str, rows: list[Any]) -> int:
    """Fill ``container[key]`` with as many of ``rows`` as fit under the byte ceiling.

    Single-pass, no per-row re-serialization of the whole envelope: measure the
    envelope once with the list empty, then add each row's own compact-JSON byte
    cost (plus one comma separator between rows) until the running total would
    exceed the budget. The per-row bytes are exact — a row embedded in a JSON
    array serializes to exactly ``compact_json(row)`` — so the estimate never
    under-counts. Returns the number of rows kept.
    """
    container[key] = []
    base = len(compact_json(reduced).encode("utf-8"))
    budget = MAX_AUDIT_RESPONSE_BYTES - base
    kept: list[Any] = []
    running = 0
    for i, row in enumerate(rows):
        cost = len(compact_json(row).encode("utf-8")) + (1 if i > 0 else 0)
...
def compact_audit_payload(payload: dict[str, Any]) -> str:
    """Serialize an audit payload under a hard final-response byte ceiling.

    Normal payload construction already bounds inventory lists and check text.
    This final guard handles synthetic or future check growth by retaining the
    verdict, counts, and as many structured result rows as fit — in a single pass,
    with one truncation step. It never emits invalid or clipped JSON.
    """
    encoded = compact_json(payload)
    if len(encoded.encode("utf-8")) <= MAX_AUDIT_RESPONSE_BYTES:
        return encoded
    ...
    reduced: dict[str, Any] = {
        ...
        if key not in {"checks", "inventory"}
    }
    reduced["response_truncated"] = True
    reduced["inventory"] = {
        key: value for key, value in inventory.items() if isinstance(value, (int, float, bool))
    }
    reduced["inventory"]["details_omitted"] = True
    reduced["checks"] = {"counts": checks.get("counts", {})}

    table = checks.get("issues")
    if isinstance(table, dict):
        issues_block: dict[str, Any] = {"columns": table.get("columns", []), "rows": []}
        reduced["checks"]["issues"] = issues_block
        rows = table.get("rows", [])
        # Reserve the count keys at their maximum width before measuring, so the
        # single-pass budget already accounts for them (returned <= total).
        reduced["results_total"] = len(rows)
        reduced["results_returned"] = len(rows)
        reduced["results_returned"] = _fit_rows(reduced, issues_block, "rows", rows)
    else:
        passed_checks = checks.get("passed_checks", [])
        reduced["results_total"] = len(passed_checks)
        reduced["results_returned"] = len(passed_checks)
        reduced["results_returned"] = _fit_rows(
            reduced, reduced["checks"], "passed_checks", passed_checks
        )

    final = compact_json(reduced)
    if len(final.encode("utf-8")) > MAX_AUDIT_RESPONSE_BYTES:
        # The fixed verdict envelope is small by construction. This fallback is
        # defensive against a future unbounded top-level field.
        final = compact_json(
            {
                "project_key": reduced.get("project_key", ""),
                "scope": AUDIT_SCOPE,
                "passed": payload.get("passed", False),
                "incomplete": payload.get("incomplete", False),
                "response_truncated": True,
                "summary": "Audit completed; detailed results exceeded the response limit",
                "checks": {"counts": checks.get("counts", {})},
            }
        )
    return final
