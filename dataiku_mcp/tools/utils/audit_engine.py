...
It separates two questions: can DSS read the Flow evidence and satisfy an
explicit delegated-output contract; and will a human find the Flow easy to
review? The first is a fail-closed gate. The second is advisory because naming,
zones, prose, visual-recipe preference, and empty graph leaves are conventions,
...
maintainability) plus an optional contract, and five statuses a :class:`Check`
can carry:
...
MAX_WIKI_CHARS_PER_ARTICLE = 100_000
...
RUNBOOK_SECTIONS = {
    ...
    "inputs": ("source", "input", "origin", "lineage"),
}
...
def _wiki_bodies(proj) -> tuple[list[str] | None, bool]:
    articles = proj.get_wiki().list_articles()
    if not articles:
        return None, False
    bodies: list[str] = []
    truncated = len(articles) > MAX_WIKI_ARTICLES
    for article in articles[:MAX_WIKI_ARTICLES]:
        data = article.get_data()
        text = f"{data.get_name() or ''}\n{data.get_body() or ''}"
        if len(text) > MAX_WIKI_CHARS_PER_ARTICLE:
            truncated = True
            text = text[:MAX_WIKI_CHARS_PER_ARTICLE]
        bodies.append(text)
...
def _flow_visible_description_check(actx: AuditContext) -> Check:
    ...
    sources, terminal = actx.inputs - actx.outputs, actx.terminal
    ...
    source_problems, source_unreadable = _description_scan(
        _dataset_metadata, proj, sorted(sources), "Source datasets"
    )
    recipe_problems, recipe_unreadable = _short_desc_scan(
        _recipe_metadata, proj, recipe_names, "Recipes"
    )
    zone_missing = _undescribed_zones(
        zones, actx.datasets | set(recipe_names)
    )
    zone_problems = (
    ...
    # Descriptions are a reviewability convention, not proof of correctness.
    # Missing or unreadable prose is therefore always advisory.
    if terminal_unreadable:
        problems.append(gap)
    if source_unreadable:
        problems.append(
            "source description evidence unreadable for: "
            + ", ".join(sorted(source_unreadable))
        )
    if recipe_unreadable:
        problems.append(
            "recipe description evidence unreadable for: "
            + ", ".join(sorted(recipe_unreadable))
        )
    if problems:
        return _bad(
...
def _wiki_units(actx: AuditContext) -> list[CheckUnit]:
    proj = actx.proj
    holder: dict[str, tuple[list[str] | None, bool]] = {}

    def snapshot() -> tuple[list[str] | None, bool]:
        if "v" not in holder:
    ...
    def wiki_scan_complete_check() -> Check:
        if not snapshot()[1]:
            return _ok("wiki_scan_complete", bucket="documentation", severity=WARN)
        return _bad(
            "wiki_scan_complete",
            bucket="documentation",
            severity=WARN,
            detail=(
                "Wiki heading scan was bounded; runbook-heading evidence may be partial "
                f"(first {MAX_WIKI_ARTICLES} articles and "
                f"{MAX_WIKI_CHARS_PER_ARTICLE} characters per article)"
            ),
            fix=(
                "Use list_wiki_articles and get_object_settings to inspect the omitted "
                "wiki articles directly."
            ),
        )
    ...
    return [
        ...
        ("wiki_scan_complete", "documentation", WARN, wiki_scan_complete_check),
    ]
...
def compact_audit_payload(payload: dict[str, Any]) -> str:
    ...
    verdict, counts, and as many structured result rows as fit. It never emits
    invalid or clipped JSON.
    """
    ...
    if isinstance(table, dict):
        ...
        rows = table.get("rows", [])
        kept: list[Any] = []
        reduced["checks"]["issues"] = {"columns": columns, "rows": kept}
        for row in rows:
            kept.append(row)
            candidate = compact_json(reduced)
            if len(candidate.encode("utf-8")) > MAX_AUDIT_RESPONSE_BYTES:
                kept.pop()
                break
        reduced["results_total"] = len(rows)
        reduced["results_returned"] = len(kept)
    else:
        passed_checks = checks.get("passed_checks", [])
        kept_passes: list[str] = []
        reduced["checks"]["passed_checks"] = kept_passes
        for check_id in passed_checks:
            kept_passes.append(check_id)
            candidate = compact_json(reduced)
            if len(candidate.encode("utf-8")) > MAX_AUDIT_RESPONSE_BYTES:
                kept_passes.pop()
                break
        reduced["results_total"] = len(passed_checks)
        reduced["results_returned"] = len(kept_passes)
