...
| `helpers.py` | `resolve_project()`, `get_client_from_ctx()` — eliminates duplication |
...
### Command Groups (17 + whoami)

| Group | File | Commands |
|---|---|---|
| `auth` | `auth_cmd.py` | login, logout, status, list, switch |
| `project` | `project.py` | list, get, export |
| `dataset` | `dataset.py` | list, schema, head, build |
| `recipe` | `recipe.py` | list, get, run |
| `scenario` | `scenario.py` | list, run, abort, status |
| `job` | `job.py` | list, status, log, abort |
| `plugin` | `plugin.py` | list, push, settings |
| `config` | `config_cmd.py` | set, get, list, path |
| `code-env` | `codeenv.py` | list, get, create, delete, update |
| `connection` | `connection.py` | list, test |
| `model` | `model.py` | list, get, versions |
| `folder` | `folder.py` | list, ls, upload, download |
| `llm` | `llm.py` | list, completion |
...
| `dku project export KEY` | `get_project(KEY).get_export_stream()` |
...
| `dku recipe get NAME` | `get_recipe(NAME).get_settings().get_raw()` |
