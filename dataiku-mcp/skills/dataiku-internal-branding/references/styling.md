...
| Core White | `dkWhite` | `#FEFEF9` | Page/app background, light surfaces |
...
<link
  href="https://fonts.googleapis.com/css2?family=Spectral:wght@400;500;600;700&family=Roboto:wght@300;400;500;700&family=DM+Mono:wght@400;500&display=swap"
  rel="stylesheet"
/>
```

```css
font-family: "Roboto", system-ui, -apple-system, sans-serif;           /* body (use font-light / font-medium for variants) */
font-family: "Spectral", Georgia, serif;                                /* headings (use font-medium for 500) */
font-family: "DM Mono", "Courier New", monospace;                       /* code/data */
```

### Font Weights

- **300 (Light):** Secondary body text, metadata
- **400 (Regular):** Body text, descriptions
- **500 (Medium):** Labels, UI emphasis, subheads
- **600–700 (Semi-bold/Bold):** Headings, titles, strong emphasis

### Font Installation

Spectral, Roboto, and DM Mono are Google Fonts — not system fonts. The company slide template (PPTX) bundles these fonts in `ppt/fonts/`, so they render on any machine without manual install. For programmatic PPTX generation (PptxGenJS), fonts must be installed on the system that opens the file. Download from [fonts.google.com](https://fonts.google.com).

System font fallbacks (safe for PPTX when brand fonts unavailable):

| Brand font | Safe fallback |
|---|---|
| Spectral | Georgia |
...
Color values: strip the `#` prefix — PptxGenJS expects bare hex strings.

```
dkGreen → '3EDAB2'  (not '#3EDAB2')
```

Images: use PNG assets — PptxGenJS does not support SVG. Logo lockups ship as both: SVG for web, **PNG for PPTX/Office**. Never recolor the logo with Signature Colors.

Font names: pass the installed font name in `fontFace` → `fontFace: 'Spectral'` (must be installed — see Font Installation above).

Recommended shadow for cards/panels:

```
{ type: 'outer', color: '000000', blur: 10, offset: 3, angle: 135, opacity: 0.14 }
```

Slide color usage:

- Cover / section dividers → background: `dkDarkGreen`, text: `dkWhite`, accent: `dkGreen`
- Content slides → background: `dkWhite`, text: `dkBlack`
- Cards / panels → fill: `dkBeige`
- CTAs / highlights → `dkGreen` on dark, `dkDarkGreen` on light

### Slide Rhythm

Use a "dark-light-dark" sandwich:

- First slide (cover) → `dkDarkGreen` background
- Content slides → `dkWhite` background
- Last slide (close/CTA) → `dkBlack` or `dkDarkGreen` background
- Section dividers (optional) → `dkDarkGreen`

Bottom accent stripe: a thin (`0.25"`) `dkGreen` bar at the foot of dark slides anchors the brand and adds polish.
