...
**Is it an app? (Phase-1 signal.)** `.yxwz` extension, OR any `AlteryxGuiToolkit.Questions.*` / `AlteryxGuiToolkit.Action.Action` node, OR `<RuntimeProperties><Questions>` → the interactive surface is a deliverable (overview.md *app keeps its interface* rule). Default to **App Designer**; a webapp only on the triggers below. Never encode the interactivity into the flow itself.
...
| `Questions.FileBrowse` | `UPLOAD_DATASET_SET_FILE` tile (or `MANAGED_FOLDER_ADD_FILE`) | user file → uploaded dataset/folder; set `exportUploads` / `exportManagedFolders` on the template or instances seed EMPTY |
...
**A render/PDF deliverable blocks only on a data wall — never on the rendering.** Chain ends in `ReportMap`/`PortfolioComposerRender`/`Render`/`Report*` → produce the artifact (`Pillow`/`reportlab`/`matplotlib` → managed folder) and validate by **artifact + shape** (file exists, right page/figure count, spot-check a page); missing tabular ground truth alone does not block. Block only when:
...
| **Image-portfolio variant** (same wall, no map) | `TextInput(name,URL)→Download→BlobConvert→Sort→PortfolioComposerRender`: block when the asset host is dead AND images aren't shipped AND there's no data-only deliverable. Doc/preview PNGs in the **solution** `.yxmd` are NOT source assets and don't unblock |
...
**Cosmetic-chrome counter-rule.** Ask: *does the analytical answer (a row-subset or classification) exist BEFORE the render?* If yes, migrate the data path; the render is chrome:
- **"Whodunit geofence"** (`RegEx(log)→…→Formula(st_distance)→Filter(dist≤R && time-window)→Download(suspect images)→PortfolioComposer`): the answer is the filtered suspect shortlist; the front-end is fully visual (`add-geopoint`+`add-geodistance`, constants via `add-formula`, ISO `YYYY-MM-DD HH:MM:SS` compares lexically). Traps: `geoDistance` uses a different earth radius than `st_distance` (~0.3% — survivor set identical unless the threshold is tight); **copy column names from `dku dataset schema`, NOT a stale terminal's `head`** — case-sensitive, wrong case silently extracts EMPTY groups.
- **Periodic-table archetype** (`Download(tool-icon PNGs, dead S3)→PortfolioComposer`): the answer is the metadata `.yxdb` classification (tool names, categories, anchor counts, flags) — migrate `Filter+Join+Summarize` against it, exact-match count per category, validate by membership not pixels. Icon PNGs shipped as `<encsection>` blobs in the `.yxdb` (columns `Image`/`Anchor Image`/`Large Background`/`Icon Images`) → full FINISH via the decode block below; block only if the metadata `.yxdb` itself is missing.
...
```
**Long-vs-short discriminator:** a yxdb column holding **long** `<encsection …PNG…>` strings → assets ARE shipped = **finish**; a column holding **short** *filenames* (join keys) → assets live at the (dead) URL = **block**. The script handles the bound trap — the blob may carry a trailing `<img src="….PNG" />` after `</encsection>`, so the base64 is bounded by the opening tag's first `>` and the **FIRST subsequent `<`** (split, not rsplit / not a greedy regex over the tail). Write decoded blobs to the managed folder keyed by a **stable row index** (`seq=0..N` → `tile_<seq>.png`), NOT a data column like a rank — ranks null/dup → filename collisions. Then `PortfolioComposer*` → Pillow render: decode each tile, composite the grid, `folder.upload_stream(...)`, validate by **artifact + shape** — no pixel diff. **Data may ship in a `.7z`** — no `7z` on stock macOS; extract with `py7zr` (`py7zr.SevenZipFile(path).extractall(dir)`).
