...
| `DateTime` | Prepare (DateParser, DateFormatter) | See `tools-state-parsing.md` and `../../dku-cli/references/prepare-processors.md` |
...
| `Summarize → CrossTab` whose only purpose is to fan a per-key value into N columns for a downstream join | Compute the N columns inline upstream with `add-formula` (one per modality); skip both the summarize and the pivot. Also avoids the modality-scan blocker (see `../../dku-cli/playbooks/tabular-flow.md`). | 2 → 0 |
...
Cross-source CLI / Dataiku gotchas live in `../../dku-cli/playbooks/tabular-flow.md`. Alteryx *language* gotchas (null propagation, `IF THEN ELSE`, `[_CurrentField_]`, `ToString(date, format)`) live in `semantics.md`.
...
| Field names often contain spaces | Quote in GREL with `numval("Customer ID")` / `strval("Customer ID")` — DSS does NOT accept Alteryx `[brackets]` and the GREL parser rejects backticks in `create-filter` formulas (see `../../dku-cli/playbooks/tabular-flow.md`) |
...
| `../../dku-cli/playbooks/tabular-flow.md` | Picking a recipe type, cross-source Dataiku/CLI gotchas, flow zones/naming, and SQL-engine push-down |
