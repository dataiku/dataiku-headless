...
Excel workbooks whose job is an *analysis pipeline*: raw data sheet → cleaned sheet → pivot tables → dashboard, and/or Power Query (M) ETL. Read `overview.md` first (triage, shared rules, parsing scripts, ingestion). Built and validated against two live migrations (Bike Buyers EDA workbook; Super Store Sales multi-file append/merge).
...
| `Source = Web.Page(Web.Contents(url))` | Live web scrape | Decide with user: pre-bake the cached table (static) or Python recipe (`pandas.read_html`) + scenario for refresh |
...
| `=RANDBETWEEN(…)`, `NOW()`, `TODAY()` | Volatile formulas | The cached value is THE value (recalcs on every open) — migrate cached data, note non-determinism |
| Hidden sheets / very-hidden sheets | Often staging or "dirt" data | Inventory them — hidden ≠ ignorable; they may feed queries |
| `.xlsm` with VBA macros | Code-driven logic | `vbaProject.bin` (extract with `oletools`/`olevba`) — inventory the macro's job; usually → Python recipe or scenario. Flag for user review |
| Slicers, conditional formatting, data validation | Presentation/UX | Slicers → dashboard filters; the rest usually drops (note it) |
...
Branch order: serial → long-format → short-format, with a final null-guard fill. Two more traps: **GREL `asDateOnly(s, fmt)` parses patterns that the `DateParser` *processor* refused outright in our runs** (all-null output for the same `M/d/yy` — if DateParser nulls everything, switch the step to a GREL formula); and the 2-digit-year pivot window (~±80y) sends corrupted years somewhere absurd (stored year 7463 renders `…63` → parses 1963) — **range-check parsed years** and repair impossible ones explicitly.
...
| `Web.Page(Web.Contents(url))` | Python recipe (`pandas.read_html`) or pre-baked cached table — user decision |
...
| `RANDBETWEEN/RAND/NOW/TODAY` | Volatile — migrate the cached values; `now()` exists in GREL but freezing the cached snapshot preserves parity |
...
| VBA macros (`.xlsm`) | Arbitrary code | Inventory intent via `olevba`; usually Python recipe or scenario step. Surface to user before migrating |
...
| Full re-detection **clobbers** manual formatParams (snaps back to the first sheet) | After hand-editing format params, re-infer columns with `detect --keep-format` — never plain `detect --save` |
...
| Editing/disabling Prepare steps without re-running `apply-schema` | Output schema goes stale → whole columns render null in `head` even though the data is fine. `apply-schema` + rebuild after EVERY step-set change |
| Phantom all-null rows (used range > table) | Blank-row filter in the first Prepare; count-match against the Table ref |
...
| Merged cells | Only the top-left cell holds the value; the rest read null → `UpDownFiller` (return `null` not `""`) for fill-down headers |
...
| Multi-input Join `-k` prefix is the join-**pair** index (pairs = inputs − 1), not the input index | 3 inputs → `-k 'Order ID' -k '1:Region'` (pairs 0..1). A wrong index fails before the recipe is created, with the pair-vs-input explanation |
...
| Sibling CSV exports are **cp1252**, not UTF-8 (Excel's default export) | DSS autodetected `windows-1252` correctly in our runs — verify (symptom: `0xa0`/accents garbled). CSV dates export in *display* format — `dd-MM-yyyy` vs `MM-dd-yyyy` is ambiguous; disambiguate from a >12 day value, ship-date coherence, or the source xlsx |
