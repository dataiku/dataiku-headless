...
4. **Wide month-columns → long format at export time**, one row per series × month, dates normalized to month-end. Reshaping is a property of the staging export script, not a Fold step in the flow.
...
- **Literal-zero histories** (rows hardcoded `0` for early columns) → zero-fill in the staging export, not a flow branch.
