#!/usr/bin/env python3
"""Dump an Excel workbook for migration analysis: formulas AND cached values.

Loads the workbook twice (data_only=False/True) and writes one TSV per sheet:

    COORD <tab> F|A|V <tab> formula-or-value [<tab> => <tab> cached-value]

F = formula, A = array formula (formula prefixed with its spilled range),
V = literal value. Also prints a per-sheet summary and flags defined names
that still resolve (broken #REF! names are counted, not listed).

Usage:
    python dump_workbook.py BOOK.xlsx OUTDIR [SHEET ...]

Requires openpyxl. Large sheets dump fine (tested ~90k cells/sheet); grep the
TSVs rather than re-opening the workbook for each question.
"""
