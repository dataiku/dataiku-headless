...
The **Tier-2** pass — Phase 3.5, on the **built** DSS graph, same rules for every source. Tier-1 (`<source>/overview.md` § Collapse triggers, Phase 2) folds *source idioms* branch-by-branch; it never sees the whole graph, so it ships graph-shape redundancy — identical sibling branches, per-metric groupings joined back, join chains. This pass catches that.
